// Main application JavaScript - Consolidated and Fixed
class FoodTruckApp {
  constructor() {
    this.events = [];
    this.filteredEvents = [];
  ...
  setupEventListeners() {
    ...
    if (truckFilter) {
      truckFilter.addEventListener('change', (e) => {
        this.filters.truck = e.target.value;
        this.applyFilters();
      });
    }
  ...
  toggleView(view) {
    ...
    document.querySelectorAll('.view-toggle').forEach(button => {
      button.classList.toggle('view-toggle--active', button.dataset.view === view);
    });
  ...
  createEventElement(event) {
    ...
    eventDiv.innerHTML = `
      ...
      <div class="event-card__content">
        ...
        ${event.confidence ? `<span class="event-card__confidence confidence--${this.getConfidenceLevel(event.confidence)}">${event.confidence}</span>` : ''}
      </div>
  ...
  populateFilters() {
    ...
    const truckFilter = document.getElementById('truck-filter');
    if (truckFilter) {
      const uniqueTrucks = [...new Set(this.events.map(event => event.truck_name))].sort();
  ...
  applyFilters() {
    this.filteredEvents = this.events.filter(event => {
      ...
      // Truck filter
      if (this.filters.truck !== 'all' && event.truck_name !== this.filters.truck) {
        return false;
      }
      ...
      if (this.filters.location) {
        ...
        const searchableText = `${event.venue} ${event.raw_address} ${event.city}`.toLowerCase();
